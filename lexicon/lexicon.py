MESSAGES: dict[str, str] = {
    '/start': 'Bienvenue à «Traducteur Rob Neuf». '
              '\n\nVous pouvez choisir la langue en appuyant sur: \n/change_language \nou utiliser les paramètres par défaut.'
              '\n\nParamètres définis maintenant: ',
    '/help': 'This is not you average translator bot.'
             '\nUse /start to start over and over again.'
             '\nUse /change_language to select source language and destination language.'
             '\nUse /help for help.',
    '/change_language': 'Select desired language for translation'
}

LEXICON_COMMANDS: dict[str, str] = {
    '/start': 'Limbo. Latin: limbus, «edge» or «boundary».',
    '/help': 'Mayday, mayday. Man overboard again.',
    '/change_language': 'Selection.'
}
